...
void print_help                (void)
{
    ...
    ("%s, version %s\n"
     ...
     " U,escape-uniqueness: 不詰と分かった代案について、Bの手数以内の詰みを免れる\n"
     "                    受方の応手が何通りあるかを数えます。全受けを試すため重くなります。\n"
     " y,yomi   : 探索中、読み筋表示。\n"
     " a,all    : 探索LOGで詰方全ての候補手の探索結果を出力します。\n"
     " [値指定]\n"
     " B,bounded-no-mate: xの追加探索でも未解決だった代案について、その代案を指して\n"
     "                    から指定手数以内に詰みが無いことを固定深さの全探索で確認し、\n"
     "                    no_mate_within_horizon として報告します。デフォルト値 0(無効)\n"
     " N,bounded-no-mate-nodes: Bの探索の1候補あたりノード上限。上限に達した候補は\n"
     "                    unresolved のまま残します。デフォルト値 0(無制限)\n"
     " S,bounded-no-mate-seconds: Bの探索に解析全体で許す秒数。デフォルト値 300\n"
     "                    (0で無制限)\n"
                                 ...
                                 tbase_t         *tbase )
{
    ...
    /* --------------------------------------------------------------------
     ...
     ------------------------------------------------------------------- */
    if(g_gc_num && mvlist->tdata.sh < list->tdata.sh && !list->inc)
    {
